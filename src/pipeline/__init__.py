...
from alchemy.pipeline import DataPipeline, ItrDataPipeline, SchedPipeline, BeginStepPipeline, EndStepPipeline
...
class ImgLoaderPipeline(ItrDataPipeline):
    def __init__(self, datapipe: ItrDataPipeline, img_path: str) -> None:
        from transformers import CLIPProcessor, CLIPImageProcessor, CLIPModel
        super().__init__(datapipe)
        model_ID = sym_tbl().cfg["model"]["vit_path"]
    ...
    def __iter__(self) -> Iterator:
        ...
        fzip: Optional[zipfile.ZipFile] = None
        def load_from_dir(name: str):
            return Image.open(self.img_path / name)
        def load_from_zip(name: str):
            with fzip.open(name) as f:
                return Image.open(f)

        if self.img_path.is_dir():
            loader = load_from_dir
        elif self.img_path.suffix == ".zip":
            fzip = zipfile.ZipFile(self.img_path, "r")
            loader = load_from_zip
        else:
            raise ValueError(f"img_path {self.img_path} must be a directory or a zip file")

        for sample in self.datapipe:
            img = loader(sample["raw"]["img_name"])
            sample["img"] = self.preprocess(img, return_tensors="pt")["pixel_values"]
            yield sample
