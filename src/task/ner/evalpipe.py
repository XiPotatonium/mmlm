...
class SaveModel(EvalPipeline):
    ...
    def __call__(self, split: str, **kwargs) -> Dict[str, Any]:
        ...
        if checkpt_dir is not None:
            if achieve_best and self.store_best:
                ...
                self._save_checkpt(save_path, **kwargs)
    ...
    def _save_checkpt(self, save_dir: Path, **kwargs):
        extra = {
        ...
        # save model
        sym_tbl().model.save(sym_tbl().model.module, save_dir)
        # save vocabulary
        sym_tbl().task.tokenizer.save_pretrained(str(save_dir))
        # save extra
